...
class DeepInsightCore:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.dr_insight = DrInsight(brave_api_key)
    ...
    def _system_init(self):
        ...
        self._log(f"🔑 Brave API: {'✅ 已配置' if self.brave.is_available() else '⚠️ 未配置'}")
    ...
    def ask(self, strategic_question: str) -> Dict:
        ...
        if self.brave.is_available():
            self._log("✅ 使用 Brave Search API 搜索全球数据")
        else:
            self._log("⚠️ Brave API 未配置，使用降级数据源")
