...
class DrInsight:
    ...
    def __init__(self, bvare_api_key: str = None):
        ...
        Args:
            bvare_api_key: Bvare API Key
        """
        # 默认使用国内搜索（避免网络问题）
        self.bvare = BvareSearch(bvare_api_key, use_domestic_only=True)
    ...
    def hunt_data(self) -> Dict:
        ...
        Returns:
            狩猎结果
        """
        if not self.bvare.is_available():
        ...
        for query in self._generate_search_queries():
            search_result = self.bvare.search(query, limit=10)
            results.append({
                "query": query,
